...
// sleep 5 minutes
#define HOUSEKEEPING_SLEEP (5 * 60) // also used for auto repeat Audio ReSync timediff > (30 mins default)
...
//		lockEvents();
	...
	if (audio_ReSync != pmsg->audio_ReSync) {
		dprintf("new audio_ReSync = %d\n", pmsg->audio_ReSync);
		writeLockEvents();
		audio_ReSync = pmsg->audio_ReSync;
		unlockEvents();
	}
	if (audio_ReSync_timer != (int)(pmsg->audio_ReSync_timer)*60) {
		dprintf("new audio_ReSync_timer = %d\n", (int)(pmsg->audio_ReSync_timer)*60);
		writeLockEvents();
		audio_ReSync_timer = (int)(pmsg->audio_ReSync_timer)*60;
		unlockEvents();
	}
...
static void *insertEventsfromDatfile(void *)
{
	...
	if (dat_file.is_open())
	{
		...
		{
			char tbuf[26];
			fprintf(logfile,"[RD-sectionsd] %s\nMemory=%dkB\nEpg days=%s\n", ctime_r(&now, tbuf), KB(meminfo.uordblks), epg_dir.c_str());
		...
		if (rd_logging)
		{
			...
			fprintf(logfile, "Processing finished in %ld seconds, %d total events loaded, %d events ignored >10MB\n"
				"\nFinished %s", tm-now, eventid, event_memory_overload, ctime_r(&tm, tbuf));
		}
