...
int eString::readEncodingFile()
{
	...
	if (f)
	{
		...
		while( getline(&line, &bufsize, f) != -1 )
		{
			...
			else if ( sscanf( line, "%s ISO%d", countrycode, &encoding ) == 2 && (encoding == 6937 || encoding == 6397) )
			...
			else if ( (sscanf( line, "0x%x 0x%x ISO%d", &tsid, &onid, &encoding ) == 3 && (encoding == 6937 || encoding == 6397) )
					||(sscanf( line, "%d %d ISO%d", &tsid, &onid, &encoding ) == 3 && (encoding == 6937 || encoding == 6397) ) )
