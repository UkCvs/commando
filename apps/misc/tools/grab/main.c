...
Based on the work of Seddi, hacked for UkCvs Commando dm500 by LraiZer.
...
vice versa. For the video, it will be grabbed directly from the decoder memory.
It also grabs the framebuffer picture in 16Bit or in 8Bit mode with the
correct colortable in 8Bit mode from the main graphics memory, because the
FBIOGETCMAP is buggy on Vulcan dm500 boxes and does not give correct color map.
Finally it will combine the pixmaps to one final picture by using the framebuffer
alphamap and save it as bmp, png file. So you will get the same picture
...
static enum {UNKNOWN, VULCAN} stb_type = UNKNOWN;
...
int main(int argc, char **argv)
{
	...
	int c,osd_only,video_only,use_osd_res,width,use_png,no_aspect,use_letterbox;

	// we use fast resize as standard now
	resize = &fast_resize;

	osd_only=video_only=use_osd_res=width=use_png=no_aspect=use_letterbox=0;
	aspect=1;
		...
		switch (c)
		{
			...
			case '?':
				fprintf(stderr,
					...
					"If no filename is given /tmp/screenshot.[bmp/png] will be used.\n");
				return 1;
			case 'o': // OSD only
				osd_only=1;
				video_only=0;
	...
	// saving picture
	if (!quiet)
			fprintf(stderr, "Saving %d bit %s ...\n",output_bytes*8, filename ? filename : "<stdout>");
	FILE *fd2;
	if (filename)
	{
...
void getosd(unsigned char *osd, int *xres, int *yres)
{
	...
	else if ( var_screeninfo.bits_per_pixel == 8 )
	{
		...
		if (stb_type == VULCAN) // DM500 stores the colors as a 16bit word with yuv values, so we have to convert :(
		{
			unsigned short yuv;
			pos2 = 0;
